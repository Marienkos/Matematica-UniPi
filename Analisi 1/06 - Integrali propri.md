...
~={cyan}DIMOSTRAZIONE=~: Caratterizzazione di inf e sup.
...
~={cyan}DIMOSTRAZIONE=~ (Darboux inclusiva $\iff$ Darboux ortodossa):
1. $S_{DI}^+(f) \leq S_{DO}^+(f)$ facendo inf
2. $S_{DO}^+(f) \leq S_{DI}^+(f)$ con osservazioni su partizione e altezze

~={cyan}DIMOSTRAZIONE=~ (Riemann $\implies$ Darboux ortodossa):
...
~={green}DIMOSTRAZIONE=~ (Darboux inclusiva $\implies$ Riemann):
...
~={cyan}DIMOSTRAZIONE=~: Integrale con estremi uguali.
