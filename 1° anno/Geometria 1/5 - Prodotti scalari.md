...
Si osserva per induzione su $n$. Se $n=1$ le isometrie sono banali. Se $n>1$, consideriamo $f$ ortogonale e $\underline{v} \in V$ tale che $f(\underline{v}) \neq \underline{v}$. Per un lemma precedente esiste una riflessione $f_{1}$ che manda $\underline{v}$ in $f(\underline{v})$, quindi $f_{1} \circ f$ fissa $\underline{v}$ e manda $W=\text{Span}(\underline{v})^\perp$ in $W$. Per ipotesi esistono trasformazioni ortogonali di $W$ la cui composizione è $(f_{1} \circ f)|_{W}$. Possiamo estenderle a riflessioni ortogonali della somma diretta ortogonale tra $\text{Span}(\underline{v})$ e $W$. Allora per ortogonalità di $f_{1}$ è verificata la tesi.